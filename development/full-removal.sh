...
#   2. ~/.ctf_env_mode     — persisted dev/prod environment choice (written by set-env)
...
show_help() {
  ...
  echo "  ~/.ctf_env_mode                Persisted dev/prod choice (if set-env was used)"
...
echo "  ${DIM}~/.ctf_env_mode    (if present — written by set-env)${RESET}"
...
# =============================================================================
# STEP 2 — REMOVE ~/.ctf_env_mode
# =============================================================================
# TEACHING NOTE — Why this file exists and why it must be removed.
#
# ~/.ctf_env_mode is written by `set-env` to persist the user's dev/prod
# environment choice across terminal sessions. It contains a single line:
#   export CTF_REPO_DIR="/opt/CTF_Public"   (or the dev path)
#
# If this file is left behind after a full removal, the next time the user
# installs and sources ~/.ctf_env, the resolution block at the top of
# ctf-env-functions.sh will find and source ~/.ctf_env_mode — immediately
# pointing CTF_REPO_DIR at a path that no longer exists (e.g. /opt/CTF_Public
# was just deleted in step 3). This causes silent, confusing failures on the
# reinstalled toolkit until the user discovers and manually removes the file.
#
# Removing it here ensures the next install starts from a clean slate, with
# auto-detection running correctly from scratch.
# =============================================================================

remove_env_mode() {
  print_step "Removing ~/.ctf_env_mode"

  local target="$TARGET_HOME/.ctf_env_mode"
  ...
  if [[ -e "$target" ]]; then
    ...
    print_skip "~/.ctf_env_mode not found — already gone (or set-env was never run)."
  fi
}
...
# =============================================================================
# STEP 6 — REMOVE CTF SYMLINKS FROM /usr/local/bin/
# =============================================================================
...
run_validation() {
  ...
  # ~/.ctf_env_mode
  if [[ ! -e "$TARGET_HOME/.ctf_env_mode" ]]; then
    print_ok "~/.ctf_env_mode — gone"
  else
    print_err "~/.ctf_env_mode — still present"
    all_clean=false
  fi
...
if $DRY_RUN; then
  ...
  echo ""
  echo "  ${CYAN}To clear exported session variables:${RESET}"
  echo "  ${BOLD}exec zsh${RESET}   ${DIM}(one-shot cleanup will run automatically)${RESET}"

  # TEACHING NOTE — Why we can't clear exported variables from a sudo subprocess,
  # and the one-shot cleanup file pattern that solves it.
  #
  # Shell variables set with `export` are in-memory state of the interactive
  # shell process. A child process (like this script running under sudo) gets
  # a *copy* of the parent's environment — changes to that copy, including
  # `unset`, never propagate back up to the parent. This is a hard Unix rule.
  #
  # Sending signals (SIGTERM, SIGKILL) to the parent shell was also unreliable:
  # SIGTERM alone was ignored, and SIGTERM+SIGKILL caused a race condition that
  # produced "zsh: killed" errors and a misaligned prompt without clearing vars.
  #
  # The one-shot cleanup file pattern solves this cleanly:
  #   1. This script writes a small cleanup file to ~/.ctf_cleanup.zsh
  #   2. It adds a single `source` line to ~/.zshrc pointing at that file
  #   3. The cleanup file runs `unset` on the next shell open (exec zsh or
  #      new terminal) — running *inside* the interactive shell that owns
  #      the variables, so the unset actually reaches them
  #   4. The cleanup file then removes its own source line from ~/.zshrc
  #      and deletes itself — it fires exactly once and leaves no trace
  #
