...
remove_zshrc_patch() {
  ...
  #
  # Four patterns run in a single pass:
  #   Expression 1: blank line + CTF comment lookahead
  ...
  #     lines in the file.
  #   Expression 2: blank line + source line lookahead
  #     Edge case: if the CTF comment was manually deleted but the blank line
  #     above the source line remains, expression 1 fires and N joins them as
  #     "\nsource ~/.ctf_env". Expression 3 won't match because ^ anchors to
  #     the start of the entire two-line pattern space (the blank line), not
  #     to the embedded source line. This expression catches that case.
  #   Expression 3: /^# CTF Toolkit/d
  ...
  #   Expression 4: /^source.*\.ctf_env/d
  #     Removes the source command line when no blank line precedes it.
  sed -i \
    -e '/^[[:space:]]*$/{N; /\n# CTF Toolkit/d}' \
    -e '/^[[:space:]]*\nsource.*\.ctf_env/d' \
    -e '/^# CTF Toolkit/d' \
...
unset ADDRESS PLATFORM BOXNAME BOX_DIR
sed -i '/# CTF session cleanup/d' "$HOME/.zshrc"
