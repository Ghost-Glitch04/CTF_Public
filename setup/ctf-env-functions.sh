...
_ctf_write_box_env() {
  # Guard: directory must exist AND be writable by the current user.
  # If the box was created with root ownership (e.g. a stale sudo mkdir),
  # writing silently fails with a zsh error. The -w check detects that
  # and skips writing rather than printing a misleading permission error.
  [[ -n "$BOX_DIR" && -d "$BOX_DIR" && -w "$BOX_DIR" ]] || return 0
...
set-platform() {
  ...
  if [[ ! -d "$pdir" ]]; then
    ...
    # to use sudo. That is a path-based heuristic, not a permission check.
    # After ctf-install.sh runs chown on /opt/CTF, the user owns it and can
    # write there without sudo — but the old check still forced sudo mkdir,
    # creating directories owned by root. The subsequent chown was supposed
    # to fix that, but could silently fail (e.g. sudo cache expired, sudo
    # restricted to specific commands), leaving dirs root-owned permanently.
    #
    # [[ ! -w "$CTF_BASE" ]] asks the OS whether the current user can write
    # to CTF_BASE right now. If yes, no sudo needed and dirs are created
    # directly as the user — no chown step required. If no, sudo is used
    # and the chown below corrects the ownership. This collapses two
...
set-box() {
  ...
  if [[ ! -d "$BOX_DIR" ]]; then
    # TEACHING NOTE — Check actual write permission, not the path prefix.
    #
    # The previous version used [[ "$CTF_BASE" == /opt/* ]] as the trigger for
    # sudo. This is a path heuristic, not a real permission check. After
    # ctf-install.sh chowns /opt/CTF to the user, the user owns the platform
    # directory and can create box directories there without sudo. Forcing sudo
    # mkdir anyway creates everything root:root, and the subsequent sudo chown
    # can silently fail (expired sudo cache, restrictive sudoers policy, etc.),
    # leaving the workspace permanently root-owned and unwritable by the user.
    #
    # We check the platform directory specifically (the immediate parent of the
    # box we are about to create). If that directory is writable, no sudo is
    # needed and dirs are created directly as the user. If not writable, sudo
    # is needed and a chown step immediately follows — atomically, in the same
    # sudo call — so no separate chown step can be skipped.
    local _parent_dir="${CTF_BASE}/${PLATFORM}"
    local use_sudo=false
    [[ ! -w "$_parent_dir" ]] && use_sudo=true
    ...
    # Fix ownership when sudo was required for the mkdir calls.
    # When use_sudo is false the dirs are already owned by the user — no chown.
    ...
    # The write is checked explicitly — a false [OK] when the write silently
    # fails (e.g. a stale root-owned directory) is worse than no message at all.
...
NOTESEOF
    local _notes_status=$?
    if [[ $_notes_status -eq 0 ]]; then
      _ctf_ok "Created starter notes: ${_CTF_BOLD}${BOX_DIR}/notes/notes.md${_CTF_RESET}"
    else
      _ctf_warn "Could not write starter notes — workspace may have a permission issue."
      _ctf_info "Fix with: ${_CTF_BOLD}sudo chown -R ${USER}:${USER} ${BOX_DIR}${_CTF_RESET}"
    fi
  else
